...
class SeverityMixDataset(torch.utils.data.Dataset):
    def __init__(self, X, y, mix_ratio=0.2):
        self.X = X
        self.y = y
    ...
    def __getitem__(self, idx):
        ...
        if np.random.rand() < self.mix_ratio:
            ...
            while self.y[rand_idx] == label:
                rand_idx = np.random.randint(0, len(self.X))
            x2, _ = self.X[rand_idx], self.y[rand_idx]
            x2 = x2.squeeze()
...
# 创建数据加载器
train_dataset = SeverityMixDataset(X_train, y_train, mix_ratio=0.2)
...
#计算混淆矩阵
confu_mat, Acc, Sen, Spe, PPV, F_score, MCC, ave_acc, ave_spe, ave_Sen = evaluate_model(gait_pre_concatenated,
                                                                                        gait_ture_concatenated,
                                                                                        class_num)
